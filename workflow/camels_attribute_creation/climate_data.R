...
##############################################################################
network_parquet      = "/Volumes/Transcend/ngen/CONUS/reference/enhd_nhdplusatts.parquet"
reference_catchments  = '/Volumes/Transcend/ngen/CONUS/reference/reference_catchments.gpkg'
reference_flowlines = '/Volumes/Transcend/ngen/CONUS/reference/reference_CONUS.gpkg'
...
camels    = readr::read_csv("data/camels_compiled.csv", show_col_types = FALSE)amels    = readr::read_csv("data/camels_compiled.csv", show_col_types = FALSE)
...
gauge_id = '01169000'; COMID = NA

tmp = dplyr::filter(camels, gauge_id == !!gauge_id | COMID == !COMID)

COMID = tmp$COMID
name  = janitor::make_clean_names(tmp$gauge_name)

dir            = hyRelease::file.path.build(base_path, name)
spatial_path   = hyRelease::file.path.build(dir, "spatial")
parameter_path = hyRelease::file.path.build(dir, "parameters")
ref = file.path(dir,"reference.gpkg")

if(!gpkg_layers(path = ref, 2, "reference")){
    hyRelease::get_UT_reference(network,
                                reference_flowlines,
